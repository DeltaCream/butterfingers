...
pub mod utils {
    ...
    #[tauri::command]
    pub fn check_fingerprint_scanner(device: State<FpDeviceManager>) -> String {
        if device.0.is_none() {
            return json!({
    ...
    #[tauri::command]
    pub fn get_device_enroll_stages(device: State<FpDeviceManager>) -> i32 {
    ...
    pub struct ManagedMySqlPool(Option<MySqlPool>);
            ...
            if self.1.is_some() {
                {
                    let cancellable = futures::executor::block_on(async {
                        self.1.as_ref().unwrap().read().await
                    });
                    cancellable.cancel();
    ...
    pub fn load_fingerprints(
        ...
        match futures::executor::block_on(async {
            managed.obtain_fingerprints_from_db(&pool).await
        }) {
            Ok(o) => json!({
                "responsecode" : "success",
                "body" : o,
    ...
    pub async fn query_employees(pool: &MySqlPool, enrolled: bool) -> Result<String, String> {
        if enrolled {
...
pub mod manage {
    ...
    #[tauri::command]
    pub fn enumerate_enrolled_employees(pool: State<ManagedMySqlPool>) -> String {
    ...
    #[tauri::command]
    pub fn delete_fingerprint(emp_id: String, pool: State<ManagedMySqlPool>) -> String {
    ...
    pub async fn delete_fingerprint_from_db(
        emp_id: &str,
        pool: &MySqlPool,
        ...
        let result =
            match sqlx::query!("DELETE FROM enrolled_fingerprints WHERE emp_id = ?", emp_id)
                .execute(pool)
                .await
            {
                Ok(result) => result,
...
pub mod enroll {
    ...
    /// For errors, it returns a JSON object with an error message.
    #[tauri::command]
    pub fn enumerate_unenrolled_employees(pool: State<ManagedMySqlPool>) -> String {
        let pool = pool.0.as_ref().unwrap();
    ...
    pub async fn save_fprint_to_db(
        emp_id: &String,
        fprint: Vec<u8>,
        pool: &MySqlPool,
    ) -> Result<(), String> {
...
pub mod attendance {
    ...
    #[tauri::command]
    pub fn manual_attendance(emp: String, pool: State<ManagedMySqlPool>) -> String {
    ...
    #[tauri::command]
    pub fn cancel_identify(managed: State<FpDeviceManager>) {
